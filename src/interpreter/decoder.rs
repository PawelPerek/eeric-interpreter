...
impl Decoder {
    ...
    pub fn decode_text_section(
        ...
        use float::{parse_r4_format as r4, parse_load_format as fl, parse_store_format as fs, parse_r_format as fr, parse_r_single_reg_format as frs,
            parse_r_to_x_format as frx, parse_r_single_reg_to_x_format as frsx, parse_r_fcvt_to_f_format as fr_to_x, parse_r_fcvt_to_x_format as fr_to_f
        };
        ...
        let instruction = match Self::rename(mnemonic) {
            ...
            "fmin.s" => Fmins(fr(op)?),
            "fmax.s" => Fmaxs(fr(op)?),
            "fcvt.w.s" => Fcvtws(fr_to_x(op)?),
            "fcvt.wu.s" => Fcvtwus(fr_to_x(op)?),
            "fmv.x.w" => Fmvxw(fr_to_x(op)?),
            "feq.s" => Feqs(frx(op)?),
            "flt.s" => Flts(frx(op)?),
            "fle.s" => Fles(frx(op)?),
            "fclass.s" => Fclasss(frsx(op)?),
            "fcvt.s.w" => Fcvtsw(fr_to_f(op)?),
            "fcvt.s.wu" => Fcvtswu(fr_to_f(op)?),
            "fmv.w.x" => Fmvwx(fr_to_f(op)?),
            "fcvt.l.s" => Fcvtls(fr_to_x(op)?),
            "fcvt.lu.s" => Fcvtlus(fr_to_x(op)?),
            "fcvt.s.l" => Fcvtsl(fr_to_f(op)?),
            "fcvt.s.lu" => Fcvtslu(fr_to_f(op)?),
            ...
            "flt.d" => Fltd(frx(op)?),
            "fle.d" => Fled(frx(op)?),
            "fclass.d" => Fclassd(frsx(op)?),
            "fcvt.w.d" => Fcvtwd(fr_to_x(op)?),
            "fcvt.wu.d" => Fcvtwud(fr_to_x(op)?),
            "fcvt.d.w" => Fcvtdw(fr_to_f(op)?),
            "fcvt.d.wu" => Fcvtdwu(fr_to_f(op)?),
            "fcvt.l.d" => Fcvtld(fr_to_x(op)?),
            "fcvt.lu.d" => Fcvtlud(fr_to_x(op)?),
            "fmv.x.d" => Fmvxd(fr_to_x(op)?),
            "fcvt.d.l" => Fcvtdl(fr_to_f(op)?),
            "fcvt.d.lu" => Fcvtdlu(fr_to_f(op)?),
            "fmv.d.x" => Fmvdx(fr_to_f(op)?),
